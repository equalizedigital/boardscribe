...
# Wait until the phpunit container can actually reach db-phpunit — not just that
# MySQL is up internally, but that the cross-container connection works.
echo "Waiting for MySQL to be reachable from phpunit container..."
MAX_TRIES=30
TRIES=0
until docker compose exec -T phpunit bash -c 'mysqladmin ping -hdb-phpunit -uwordpress -pwordpress --silent' 2>/dev/null; do
  TRIES=$((TRIES+1))
  if [ "$TRIES" -ge "$MAX_TRIES" ]; then
    echo "MySQL did not become reachable in time. Exiting."
    exit 1
  fi
  sleep 2
done
