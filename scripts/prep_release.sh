...
MAIN_FILE_PATH='boardscribe.php'
PACKAGE_JSON_PATH='package.json'
README_PATH='readme.txt'

echo "Getting plugin version from ${MAIN_FILE_PATH}"
VERSION=$(grep "Version:" ${MAIN_FILE_PATH} | sed -E 's/ \* Version:[[:space:]]*([0-9]+\.[0-9]+\.[0-9]+).*/\1/')
...
sed -i.bak -E "s/(define\( 'EDBS_VERSION', ')[0-9]+\.[0-9]+\.[0-9]+/\1${BUMPED_VERSION}/" "${MAIN_FILE_PATH}"
...
echo "https://github.com/equalizedigital/boardscribe/compare/${MAIN_BRANCH}...${RELEASE_BRANCH_NAME}?title=Release+v${BUMPED_VERSION}"
