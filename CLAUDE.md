...
*(The four hooks above are on branch `feature/pro-extension-hooks` / PR #9, not yet merged to `main` as of this writing.)*

**Known gap:** there's currently no override point for the table's *rendering strategy* itself (accordion/card-grid/timeline templates) — only column additions via `window.edmmExtraColumns`. This is a known open question, not yet resolved.
