...
includes/
  ...
  Admin/SettingsPage.php               Settings page + shortcode builder page (enqueues the React builder app)
...
| `edbs_cpt_args` | filter | `PostType/BoardScribeCPT.php` | Modify CPT registration args (e.g. enable `public`/`rewrite`/`has_archive`, or a custom `capability_type`) before `register_post_type()`. `public` defaults to `false` for now (paused — see GH#25/PRO-1209: making it `true` is how a plugin like ArchiveWP, which keys its own features off that flag directly, can see this CPT; there's no other extension point for it), `rewrite`/`has_archive` default to `false` regardless. |
