...
| `edmm_shortcode_builder_fields` | action | `Admin/SettingsPage.php` | Add wholly new rows to the shortcode builder UI (e.g. a template selector) — for extending the *existing* Column Labels / Hide Columns rows instead, use the two filters below so Pro's fields land in the same section rather than a duplicate one. The generic builder script in `SettingsPage::get_builder_script()` walks every named field itself (no matching JS needed) — a checked checkbox becomes `name="value-or-true"`; any other field becomes `name="value"` when non-empty and different from its `data-default` attribute (empty string if omitted). |
| `edmm_shortcode_builder_label_fields` | filter | `partials/settings-page.php` | Add entries (field name => default label) to the builder's "Column Labels" row. Pro appends its own column labels (location, livestream, etc.) here instead of rendering a second "Column Labels"-like section. |
| `edmm_shortcode_builder_hide_fields` | filter | `partials/settings-page.php` | Add entries (field name => checkbox label) to the builder's "Hide Columns" row, same rationale as the filter above. |
