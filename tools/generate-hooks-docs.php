 ...
 *
 * Only hooks that are plugin-specific (prefixed with `edbs_`) are included.
 *
 ...
 */
function edbs_find_nearest_docblock( $contents, $pos, $max_lines = 10 ) {
...
/**
 * Scans the repository for edbs_-prefixed hooks and writes docs/hooks.md.
 *
 * Kept in a single function (rather than the previous top-level script)
 * so every working variable below is function-scoped instead of global —
 * this repo's phpcs.xml enforces WordPress.NamingConventions.PrefixAllGlobals
 * on true globals, which a procedural CLI script would otherwise trip on
 * for every local-in-spirit variable it declares.
 *
 * @return void
 */
function edbs_generate_hooks_docs() {
	...
	$github_repo   = getenv( 'GH_REPO' ) ? getenv( 'GH_REPO' ) : 'boardscribe';
	...
	foreach ( $rii as $file ) {
		...
		$contents = file_get_contents( $filepath ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- CLI build tool reading local source files, not a WordPress runtime context.
		...
		if ( preg_match_all( '/add_(action|filter)\s*\(\s*([\'\"])([^\2]+?)\2/', $contents, $matches_rows, PREG_SET_ORDER ) ) {
			foreach ( $matches_rows as $row ) {
				...
				if ( preg_match( '/^(edbs_)/', $hook_name ) ) {
					...
					$doc_info = edbs_find_nearest_docblock( $contents, $pos, 6 );
	...
	$out  = "# BoardScribe Hooks Reference\n\n";
	$out .= "This document is auto-generated by `tools/generate-hooks-docs.php`. It lists only plugin-specific PHP hooks (prefixed with `edbs_`) and links files to the plugin's GitHub branch. It does not cover the JavaScript registries/events or reusable PHP methods documented in README.md's Developer Hooks section.\n\n";
	...
	file_put_contents( $docs_path, $out ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents -- CLI build tool writing a local doc file, not a WordPress runtime context.

	printf( "Generated docs/hooks.md with %d hooks.\n", count( $hooks ) );
}

edbs_generate_hooks_docs();
