...
export function defaultRenderPagination( data, instanceCfg, element, goToPage ) {
	...
	if ( data.max_num_pages > 1 ) {
		pagination += '<nav aria-label="' + escapeAttribute( i18n.pagination || __( 'Pagination', 'equalize-digital-meeting-minutes' ) ) + '">' +
			'<div class="edmm-pagination-buttons">';

		if ( data.current_page > 1 ) {
			pagination += '<button type="button" class="edmm-pagination-button prev"' +
				' aria-label="' + escapeAttribute( i18n.previousPage || __( 'Previous Page', 'equalize-digital-meeting-minutes' ) ) + '"' +
				' data-page="' + ( data.current_page - 1 ) + '">' +
				escapeHTML( i18n.previous || __( 'Previous', 'equalize-digital-meeting-minutes' ) ) +
				'</button>';
		}
		...
		slots.forEach( function( slot ) {
			if ( slot === '...' ) {
				...
				const label = escapeAttribute( ( i18n.pageNum || __( 'Page %s', 'equalize-digital-meeting-minutes' ) ).replace( '%s', slot ) );
		...
		if ( data.current_page < data.max_num_pages ) {
			pagination += '<button type="button" class="edmm-pagination-button next"' +
				' aria-label="' + escapeAttribute( i18n.nextPage || __( 'Next Page', 'equalize-digital-meeting-minutes' ) ) + '"' +
				' data-page="' + ( data.current_page + 1 ) + '">' +
				escapeHTML( i18n.next || __( 'Next', 'equalize-digital-meeting-minutes' ) ) +
				'</button>';
		}
