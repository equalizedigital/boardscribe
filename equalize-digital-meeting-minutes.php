 ...
 * Text Domain:       equalize-digital-meeting-minutes
	...
	add_action(
		...
		function () {
			...
			. sprintf(
				...
				esc_html__( 'Equalize Digital Meeting Minutes requires PHP %1$s or higher. You are running PHP %2$s. Please upgrade PHP or contact your host.', 'equalize-digital-meeting-minutes' ),
				'7.4',
				PHP_VERSION
			)
			. '</p></div>';
...
if ( ! file_exists( EDMM_DIR . 'vendor/autoload.php' ) ) {
	add_action(
		...
		function () {
			echo '<div class="notice notice-error"><p>'
			. esc_html__( 'Equalize Digital Meeting Minutes is missing its autoloader. Reinstall from a released zip, or run `composer install` if developing from source.', 'equalize-digital-meeting-minutes' )
			. '</p></div>';
		}
	);
	return;
